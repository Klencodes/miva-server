...
class InventoryController {
  ...
  getItems = async (req, res) => {
    try {
      ...
      if (!entityId) {
        console.warn("⚠️ No entity ID found in request");
        // Return empty results with clear message
        const response = new ApiResponse(
          [],
          "No entity ID provided. Please select an entity.",
          {
            count: 0,
  ...
  getItemByPartNumber = async (req, res) => {
    try {
      ...
      const item = await InventoryService.getItemByPartNumber(
        entityId,
        partNumber,
      );
  ...
  bulkCreateItems = async (req, res) => {
    try {
      ...
      const response = new ApiResponse(
        result,
        "Bulk inventory creation completed",
      );
      return res.json(response);
    } catch (error) {
      return this.handleError(error, res);
