...
class UserController {
  ...
  removeEntityFromUser = async (req, res) => {
    try {
      ...
      const user = await UserService.removeEntityFromUser(
        uuid,
        entity_id,
        req
      );
  ...
  setPrimaryEntity = async (req, res) => {
    try {
      ...
      const user = await UserService.setPrimaryEntity(
        uuid,
        entity_id,
        req
      );
    ...
    try {
      ...
      return res.json({
        ...
        results: {
          ...
          permissions_array: user.permissions ? Object.keys(user.permissions).filter(key => user.permissions[key]) : []
        }
      });
    } catch (error) {
      return this.handleError(error, res);
  ...
  handleError(error, res) {
    ...
    const errorMap = {
      ...
      'ENTITY_NOT_FOUND': {
        status: 404,
        message: 'Entity not found in user\'s list',
